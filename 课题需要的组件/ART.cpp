...
int main(int argc,char **argv){
    int i;
    unsigned char** cc=new unsigned char*[N];
    size_t **value = new size_t *[N];
    typedef std::chrono::steady_clock Clock;
    for(i=0;i<N;i++){
       cc[i]=(unsigned char *)new string(rand_str(8));
       value[i]=new size_t(i*2+1);

    }
    bool RIGHT=true;
    int *ide;
    art_tree *my_tree =new art_tree;
    art_tree_init(my_tree);
    auto t1 = Clock::now();//计时结束
    for(i=0;i<N;i++){
        art_insert(my_tree,cc[i],12,value[i]);
    }
    auto t2 = Clock::now();//计时结束
    auto t3 = Clock::now();//计时结束
    for(i=0;i<N;i++){
       ide=(int *)art_search(my_tree,cc[i],12);
       if((*ide)!=i*2+1){
           RIGHT=false;
        //   break;
       }
    ...
    if(RIGHT){
        ...
        cout<<"that's an error here."<<endl;
    }
    std::cout<<N<<"次写操作延迟:" <<float(std::chrono::duration_cast<std::chrono::nanoseconds>(t2 - t1).count())/1000000<<"ms"<<'\n';
    std::cout<<N<<"次读操作延迟:" <<float(std::chrono::duration_cast<std::chrono::nanoseconds>(t4 - t3).count())/1000000<<"ms"<<'\n';

    art_tree_destroy(my_tree);
